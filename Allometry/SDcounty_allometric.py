...
# [markdown]
# The data set "all_clean.csv" was converted to a shapefile in QGIS (3.10.4-A) and clipped
# to only contain points within San Diego County using a shapefile of the San Diego
# city boundry downloaded from
# https://catalog.data.gov/dataset/tiger-line-shapefile-2018-county-san-diego-county-ca-topological-faces-polygons-with-all-geocod
# and the tree inventory dataset. The shapefile had municipalities, so it was first dissolved
# in QGIS before the clip.  
#
# 
...
# read the csv
data = pd.read_csv(path, usecols=['ID', 'LATITUDE', 'LONGITUDE',
...
# drop NAs
data.dropna(how='any', axis=0, subset=['DBH_LO', 'DBH_HI'], inplace=True)
#%% [markdown]
...
data['temp_range'] = temp_range(data.DBH_LO, data.DBH_HI)

data.temp_range.value_counts()
...
#%%
data.drop('temp_range', axis=1, inplace=True) #housekeeping
data['sim_dbh'] = np.random.uniform(low=data.DBH_LO, high=data.DBH_HI)
...
data['dbh_range'] = [get_range(x) for x in data.sim_dbh.values]
...
count = data[['ID', 'dbh_range']].groupby(by=['dbh_range']).count()
...
_ = br_plt.set(ylabel='DBH range (inches)', title = 'DBH range of all tree species - SD County')
plt.savefig('{}hist_SDCounty_all.pdf'.format(fig_path))
...
species_list = list(data.Name_matched.unique())

top_ten = list(data.Name_matched.value_counts().head(10).index)
top_ten.sort()
t10 = data.loc[data.Name_matched.isin(top_ten)][['Name_matched', 'dbh_range']]
t10['Name_matched'] = t10.Name_matched.str.capitalize()
...
plt.xlabel('Species')
plt.title('DBH range of the ten most frequently occuring tree species in SD County')
plt.tight_layout()
plt.savefig('{}SDCounty_top_ten.pdf'.format(fig_path))
